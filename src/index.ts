...
program
  ...
  .action(async (options) => {
    try {
      ...
      console.log(chalk.yellow("Note: Creating posts via CLI minimal wrapper."));
      console.log("Payload:", JSON.stringify(payload, null, 2));
      
      const result = await api.createPost(payload);
      console.log(chalk.green('Post created:'), result);
    } catch (error: any) {
      console.error(chalk.red('Error:'), error.response?.data || error.message);
    }
  });
